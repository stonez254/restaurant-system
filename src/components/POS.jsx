...
export default function POS({menu,categories,modifiers,cart,addItem,changeQty,removeCart,placeOrder,channel,setChannel,selectedTable,setSelectedTable,tables,money,customers,setCustomerId,customerId,editingOrderId,cancelEdit}){
 ...
 <aside className="cart-panel">{editingOrderId&&<div className="edit-banner">Editing {editingOrderId}<button onClick={cancelEdit}>Cancel</button></div>}
  <div className="cart-head"><div><h3>{editingOrderId?"Edit Order":"Current Order"}</h3><small>{cart.reduce((s,x)=>s+x.qty,0)} items</small></div><ReceiptText size={20}/></div>
  ...
  {channel==="Dine In"&&<><select className="select" value={selectedTable?.id||""} onChange={e=>setSelectedTable(tables.find(x=>x.id===Number(e.target.value))||null)}><option value="">Select table</option>{tables.filter(t=>t.status==="Vacant"||(selectedTable&&t.id===selectedTable.id)||t.orderId===editingOrderId).map(t=><option value={t.id} key={t.id}>{t.name} • {t.seats} seats{t.mergedGroupId?" • merged":""}</option>)}</select>
   <select className="select" value={customerId||""} onChange={e=>setCustomerId(e.target.value||null)}><option value=""><UserRound size={12}/> Walk-in customer</option>{customers.map(c=><option key={c.id} value={c.id}>{c.name} • {c.phone}</option>)}</select></>}
  ...
  <div className="cart-total"><span>Total</span><strong>{money(total)}</strong></div><button className="checkout" disabled={!cart.length||channel==="Dine In"&&!selectedTable} onClick={placeOrder}>{editingOrderId?"Save Changes":"Place Order"}</button>
 </aside></section>
}